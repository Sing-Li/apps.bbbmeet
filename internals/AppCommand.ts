...
export class AppCommand implements ISlashCommand {
        ...
        this.executor = async (
            ...
            await this.handleCommands(
                ...
                }: // tslint:enable: no-shadowed-variable
                {
                    ...
                    if (args.length === 0) {
                        return await __executor(context, read, modify, http, persis)
                    }
                    return await this.showHelp({context, read, modify, http, persis}, args)
                },
                {context, read, modify, http, persis}
            )
        }
    ...
    protected accessibleSubcommand(sender: IUser, subcommand: string): AppCommand | undefined {
    ...
    protected async handleSubcommands(
        ...
        if (commandArgs.length === 0) {
            return await commandHandler.executor(context, read, modify, http, persis, commandArgs)
        }
